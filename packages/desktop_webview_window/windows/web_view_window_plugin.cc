...
WebviewWindowPlugin::~WebviewWindowPlugin() = default;void WebviewWindowPlugin::HandleMethodCall(
    ...
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
{
  ...
  else if (method_call.method_name() == "updateWindow")
  {
    auto *arguments = std::get_if<flutter::EncodableMap>(method_call.arguments());
    auto window_id = arguments->at(flutter::EncodableValue("viewId")).LongValue();
    ...
    auto x = arguments->at(flutter::EncodableValue("x")).LongValue();
    auto y = arguments->at(flutter::EncodableValue("y")).LongValue();
    auto index = arguments->at(flutter::EncodableValue("index")).LongValue();
    if (!windows_.count(window_id))
    {
      result->Error("0", "can not find webview window for id");
      return;
    ...
    windows_[window_id]->updateWindow(int(x),int(y),int(width),int(height),int(index));
    result->Success();
  }
  else if (method_call.method_name() == "showWebview")
  {
    auto *arguments = std::get_if<flutter::EncodableMap>(method_call.arguments());
    auto window_id = arguments->at(flutter::EncodableValue("viewId")).LongValue();
    auto showWebView = arguments->at(flutter::EncodableValue("showWebView")).LongValue();
    if (!windows_.count(window_id))
    {
      result->Error("0", "can not find webview window for id");
      return;
    ...
    windows_[window_id]->showWebView(showWebView>0?true:false);
    result->Success();
  }
  else
  {
